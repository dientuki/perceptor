...
— and again 2026-09-14 after `056-shorts-runtime-classification`: `api` 483/44 suites, 0 typecheck
errors, `git status --short services/api/prisma` empty (no migration — `Movie.isShort` already
existed). `web` typechecks at 0 errors, `bin/npm web run build` exits 0, and
...
— and again 2026-09-17 after `059-season-pack-acquisition-ui`: `api` 536/46 suites, 0 typecheck
errors, `git status --short services/api/prisma` empty (no migration — NFR-1, every column REQ-7
needs already existed). `web` typechecks at 0 errors, `bin/npm web run build` exits 0, and
`bin/cli web node scripts/check-messages.mjs` confirms no `en`/`es` drift at 428 keys. `worker`
untouched (`git diff --stat services/worker` empty, NFR-4). `schema.gql` diff is exactly the one
mutation in `spec.md`'s contract delta, `addTorrentToSeason`. Manual pass on `/shows/<id>`
confirmed AC-1, AC-2, AC-6 and AC-8 live against a running stack (season header buttons, a season
...
— and again 2026-09-19 after `060-duplicate-torrent-add`: `api` 559/46 suites, 0 typecheck errors,
