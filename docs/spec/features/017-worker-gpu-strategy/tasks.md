...
- [ ] **T001** `[infra]` Add `- USE_GPU=${USE_GPU}` to the `worker` service's `environment:` block
...
- [ ] **T002** `[docs] [P]` Correct `.claude/agents/worker.md`, which still states this service has
...
- [ ] **T003** `[worker] [P]` Create `src/ffmpeg/vulkan.ts`: async `probeVulkan()` returning both a
...
- [ ] **T004** `[worker]` Write `src/ffmpeg/vulkan.spec.ts`, opening with a header comment naming
...
- [ ] **T005** `[worker] [P]` Add `vulkanAvailable: boolean` to `getVideoParams` in
...
- [ ] **T006** `[worker]` Extend `src/ffmpeg/params.spec.ts` with the first-ever `getVideoParams`
...
- [ ] **T007** `[worker]` Add the boolean as a fifth parameter of `buildFfmpegCommand`
...
- [ ] **T008** `[worker]` Wire it up: `src/encode/encode.ffmpeg.ts` calls `isVulkanAvailable()` and
...
- [ ] **T009** `[infra] [P]` Replace the `USE_GPU` block in `bin/dev:21`, `bin/prod:22` and
...
- [ ] **T010** `[infra] [P]` Delete the GPU question and its explanatory `echo` block from
...
- [ ] **T011** `[infra] [P]` In `services/worker/Dockerfile`, add the Mesa AMD Vulkan driver to the
...
- [ ] **T012** `[infra] [P]` Rewrite the `USE_GPU` comment block in `.env.example:20-27` for the new
...
- [ ] **T013** `[worker]` Walk **AC-1 and AC-2** against the `/tmp/hdr4k.mkv` fixture from T005: with
...
- [ ] **T014** `[infra]` Walk **AC-3 and AC-4**: bring the stack up without the overlay so the
...
- [ ] **T015** `[docs]` Update the prose surfaces: `README.md:58` (the "optional GPU acceleration
...
- [ ] **T016** `[docs]` Walk the acceptance criteria in `spec.md`, tick each box that is genuinely
