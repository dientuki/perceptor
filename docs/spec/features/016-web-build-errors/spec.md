...
- [x] **REQ-1 (Green build)**: `bin/npm web run build` must exit 0.
- [x] **REQ-2 (Clean typecheck)**: `web`'s typecheck must report **0 errors** (today 11 across 4
      files).
- [x] **REQ-3 (No suppression)**: No `typescript.ignoreBuildErrors` added to `next.config.ts`, no
      `@ts-ignore`/`@ts-expect-error`, and no explicit `any` used to silence the compiler.
- [x] **REQ-4 (No functional regression)**: No screen that works today may change behaviour. The 4
      files are unreachable from the current UI; if any is kept, it must end up compiling and
      functionally equivalent.
- [x] **REQ-5 (Per-file decision)**: For each of the 4 files, the implementation must explicitly
...
- [x] **REQ-6 (Prerender)**: `next build` must complete the "Generating static pages" stage without a
...
- [x] **REQ-7 (Root cause, not symptom)**: The fix must address **why** the dispatcher arrives
      ...
      **not** an acceptable fix: it was tested during planning and only moves the failure to the next
      page.
- [x] **REQ-8 (No suppression, extended)**: REQ-3's ban extends here — the prerender stage is not
      disabled, `ignoreBuildErrors` is not added, and no page is deleted to make the build pass.

### Non-Functional & Operational Requirements

- [x] **NFR-1 (Contract untouched)**: `fetchGraphQL` and every existing query/mutation stay as they
      are; this is dead-code cleanup plus a build fix, not a contract change.
- [x] **NFR-2 (Article II)**: `grep -rn "@prisma/client" services/web/src` keeps returning nothing.
- [x] **NFR-3 (Documentation)**: The error table in `services/web/CLAUDE.md` and the "Current state"
...
- [x] **AC-1**: `bin/npm web run build` exits 0.
- [x] **AC-2**: `bin/cli web npx --no tsc --noEmit` reports 0 errors.
- [x] **AC-3**: `grep -rn "ignoreBuildErrors\|@ts-ignore\|@ts-expect-error" services/web` returns
      nothing.
- [x] **AC-4**: `grep -rn "@/actions/jobs\|@/icons" services/web/src` returns nothing.
- [x] **AC-5 (failure path)**: Introducing a deliberate type error in
...
- [x] **AC-6**: `bin/dev` brings `web` up and `/movies`, `/movies/<id>`, `/shows` and `/shows/<id>`
      render as before, including the three per-episode buttons.
- [x] **AC-7**: `bin/npm web run build` reaches "Generating static pages" and passes it: its output
      contains no `Error occurred prerendering page` line.
- [x] **AC-8**: `grep -rn "force-dynamic" services/web/src/app` returns nothing — no page was excused
