...
- [x] **T001** `[api] [P]` Add `isShort Boolean @default(false)` to `model Movie` in
      ...
      *Done when:* `bin/cli api npx prisma migrate status` reports the new migration applied with no
      ...
      directory.
- [x] **T002** `[api] [P]` Register `shorts_enabled: { kind: 'boolean' }` and
...
- [x] **T003** `[api] [P]` Add `MEDIA_SHORTS_DISABLED: 'error.media.shorts_disabled'` and
...
- [x] **T004** `[api]` Extract `MediaCapabilitiesService` into a new
...
- [x] **T005** `[api]` Add the optional `asShort` argument to `addMedia` in
...
- [x] **T006** `[api]` Add `MoviesService.setShort(id, userId, isShort)` (ownership through the
      ...
      *Done when:* with shorts enabled, `setMovieShort` on an owned film flips
      ...
      column is unchanged.
- [x] **T007** `[api]` Add `isShort: Boolean!` to `movies/entities/movies.entity.ts`, add the
      ...
      *Done when:* `bin/npm api test` passes; `movies` with no argument returns every film the caller
      ...
      the rest.
- [x] **T008** `[api]` Add `isShort: Boolean!` to
...
- [x] **T009** `[api]` In `ProcessJobsService.getEncodeJobDetails`'s film arm, resolve `outputRoot`
...
- [x] **T010** `[web]` Add `shortsEnabled: boolean` to `MediaCapabilities` in `src/types/media.ts`
      ...
      *Done when:* `bin/npm web run build` exits 0 and a dashboard render logs no GraphQL error for
      an unknown field.
- [x] **T011** `[web] [P]` Add the **Shorts** entry to `AppSidebar.tsx`'s `baseNavItems`, between
...
- [x] **T012** `[web] [P]` Add the optional `isShort` argument to `getMovies` in
      ...
      *Done when:* `/shorts` renders the not-found page with shorts off; with shorts on it lists only
      ...
      again.
- [x] **T013** `[web] [P]` Add `isShort` to the search-result type in `src/types/search.ts` and to
...
- [x] **T014** `[web]` Add the optional `asShort` to `addMedia` in `src/actions/media.ts` and an
...
- [x] **T015** `[web] [P]` Add `setMovieShortAction(movieId, isShort)` beside the other per-title
...
- [x] **T016** `[web] [P]` Add the shorts `Switch` + hidden input and the `path_shorts` `PathPicker`
      ...
      *Done when:* the shorts row is greyed while Movies is off and live when it is on; saving the
      ...
      unchanged.
- [x] **T017** `[web]` Reconcile `messages/en.json` and `messages/es.json` for every string added in
...
- [x] **T018** `[docs]` Add a `048-shorts-category` section to `docs/spec/graphql-contract.md`
...
- [x] **T019** `[docs]` Update the affected `CLAUDE.md` files: the root pipeline table (the
...
- [x] **T020** `[docs]` Run the full verification block from `plan.md` § Verification, walk the
