...
- [ ] **REQ-1 (The flag)**: A film must carry a boolean `isShort`, `false` by default. It is a
...
- [ ] **REQ-2 (Two new settings)**: Settings must carry `shorts_enabled` (boolean, seeded `false`)
...
- [ ] **REQ-3 (Subordinate to movies)**: The **effective** shorts capability is
...
- [ ] **REQ-4 (Capability query)**: `mediaCapabilities` must expose the effective shorts capability
      alongside the existing two, so every `web` surface reads one value and never re-derives the
      `&&` itself.
- [ ] **REQ-5 (Register as a short)**: While shorts are enabled, a film search result must offer
...
- [ ] **REQ-6 (Toggle from the detail page)**: While shorts are enabled, the film detail page
...
- [ ] **REQ-7 (Search badge)**: While shorts are enabled, a search result already registered as a
...
- [ ] **REQ-8 (Sidebar and listings)**: While shorts are enabled, the sidebar must show a **Shorts**
...
- [ ] **REQ-9 (One detail page)**: A card in `/shorts` links to `/movies/[id]`. There is no
      `/shorts/[id]`: the detail page, its torrent search modal, its upload modal and its language
      preferences are reused unchanged.
- [ ] **REQ-10 (Route gating)**: While shorts are disabled, `/shorts` must answer the existing
      not-found page, exactly as `/shows` does with series disabled (`045` REQ-6).
- [ ] **REQ-11 (Disabled means invisible, not lost)**: While shorts are disabled, `isShort` has no
...
- [ ] **REQ-12 (Output root)**: A `ProcessJob` for a film with `isShort: true` must resolve its
...
- [ ] **REQ-13 (Reclassification is not a move)**: Flipping `isShort` never moves, renames, copies or
...
- [ ] **REQ-14 (Enforcement, not just hiding)**: `api` must refuse to register a title as a short, and
...
- [ ] **REQ-15 (Nothing else is gated)**: Acquisition, download progress, the tus upload route,
...
- [ ] **NFR-1 (No new external calls)**: This feature adds no TMDB request on any path. The search
      ...
      request per row.
- [ ] **NFR-2 (Worker untouched)**: No file under `services/worker/` changes. `outputRoot` is already
...
- [ ] **NFR-3 (Migration is additive)**: `Movie.isShort` is added non-null with a `false` default, so
...
- [ ] **NFR-4 (Path safety)**: `path_shorts` is a `kind: 'path'` catalog entry against the `library`
...
- [ ] **NFR-5 (One capability read per render)**: A page render reads `mediaCapabilities` once and
      passes it down, reusing the `cache()`-wrapped `getMediaCapabilities()` of `045` rather than
      adding a second fetch.
- [ ] **NFR-6 (System-wide)**: Like `movies_enabled`/`shows_enabled`, the shorts capability is
...
- [ ] **AC-1**: After `bin/dbreset`, `bin/mysql -e "select value from settings where \`key\` in ('shorts_enabled','path_shorts')"` returns `false` and `Shorts`; the sidebar shows no **Shorts** entry and `/shorts` renders the not-found page.
- [ ] **AC-2**: Turning `shorts_enabled` on in Settings → Media Manager and reloading makes the **Shorts** sidebar entry and the `/shorts` route appear, with `/movies` still listing every film (all of which are `isShort: false` at this point).
- [ ] **AC-3**: With shorts enabled, searching a short film and registering it as a short lands it in `/shorts` and not in `/movies`, and `bin/mysql -e 'select isShort from movies order by id desc limit 1'` returns `1`.
- [ ] **AC-4**: With shorts enabled, opening an already-registered feature film's `/movies/[id]`, flipping the shorts toggle on and reloading moves the card from `/movies` to `/shorts`, and re-searching that title in `/search` now shows it badged as a short.
- [ ] **AC-5**: Acquiring and completing a film flagged `isShort` files the output under the `path_shorts` folder (`Shorts/<Title> (<year>) [tmdbid=…]/…`), while a film not flagged, completed in the same session, still lands under `path_movies`.
- [ ] **AC-6 (failure path)**: With `shorts_enabled` off, a hand-issued `setMovieShort(movieId: <id>, isShort: true)` is rejected with `extensions.i18n.key = "error.media.shorts_disabled"` and writes nothing: `bin/mysql -e 'select isShort from movies where id = <id>'` returns the same value before and after. The same call with `movies_enabled` off is rejected with `error.media.type_disabled`.
- [ ] **AC-7 (failure path)**: A hand-issued `addMedia(tmdbId: 1399, type: "show", asShort: true)` is rejected with `error.media.shorts_not_a_movie` and creates no row in `shows`.
- [ ] **AC-8 (failure path)**: A hand-issued `setMovieShort` naming a film registered by a **different** user is rejected with `error.movie.not_found`, indistinguishable from a film id that does not exist.
- [ ] **AC-9 (nothing is moved)**: Flipping `isShort` on a film already `COMPLETED` leaves its file exactly where it is — the path under `path_movies` still exists, no file appears under `path_shorts`, and the film's stored `filePath` is unchanged.
- [ ] **AC-10 (regression)**: With shorts enabled and at least one film flagged, turning `movies_enabled` off hides Movies **and** Shorts from the sidebar, makes both routes answer the not-found page, and leaves both stored settings untouched (`bin/mysql -e "select value from settings where \`key\` = 'shorts_enabled'"` still returns `true`); turning movies back on restores both.
- [ ] **AC-11**: `bin/npm api test` passes, `bin/cli api npx --no tsc --noEmit` reports 0 errors, and `bin/npm web run build` exits 0. `git status services/worker` is clean, per NFR-2.
