...
- [x] **T001** `[api]` Add `ENCODE_CANCEL_CHANNEL = 'encode:cancel'` and
...
- [x] **T002** `[api]` Add `removeSourceReady(mediaSourceId)` to `process-queue.service.ts` and
...
- [x] **T003** `[api] [P]` Add `isInsideRoot(rootId, absolutePath): Promise<boolean>` to
...
- [x] **T004** `[api] [P]` Add `toMediaStatus(status: PipelineStatus): MediaStatus` to
...
- [x] **T005** `[api]` Rewrite `downloadDelete` in `downloads.service.ts` as the orchestrator:
...
- [x] **T006** `[api]` Add the on-disk deletion helper called from step 5 of `downloadDelete`:
...
- [x] **T007** `[api]` Add the status recompute called from step 7 of `downloadDelete`: resolve the
...
- [x] **T008** `[api]` Extend `downloads.service.spec.ts` for the deletion orchestration: an upload
...
- [x] **T009** `[worker] [P]` Transcribe `ENCODE_CANCEL_CHANNEL` and `EncodeCancelMessage` into
      ...
      *Done when:* `diff <(grep -A2 "encode:cancel" services/api/src/queue/types.ts) <(grep -A2
      ...
      the field name.
- [x] **T010** `[worker] [P]` Add `src/encode/cancellation.ts` — the
...
- [x] **T011** `[worker]` Open one ioredis subscriber on `ENCODE_CANCEL_CHANNEL` in `src/index.ts`,
      ...
      *Done when:* `docker compose logs worker` shows the subscription at boot, and
      ...
      the worker stays up.
- [x] **T012** `[worker] [P]` Add `signal: AbortSignal` as a **required** parameter of `EncodeFn`
...
- [x] **T013** `[worker]` Wire abort into `src/ffmpeg/runner.ts`: reject immediately with
      ...
      *Done when:* with an encode running, `bin/cli redis redis-cli PUBLISH encode:cancel
      ...
      root (AC-5).
- [x] **T014** `[worker]` In `src/jobs/encode.job.ts`, call `registerEncode(processJobId)` before
...
- [x] **T015** `[web]` In `messages/en.json` and `messages/es.json`, replace
...
- [x] **T016** `[web] [P]` In `DownloadsPanel.tsx`, split the `isControllable` gate: Play and
...
- [x] **T017** `[web] [P]` In `DeleteDownloadModal.tsx`, pick `messageTorrent` vs `messageUpload`
...
- [x] **T018** `[docs]` Add a `047-source-deletion` section to `docs/spec/graphql-contract.md`:
      ...
      → T008, T014
- [x] **T019** `[docs]` Update the `CLAUDE.md` files: the root pipeline table's **Download** and
      **Transcode** rows (a delete now unwinds queued and running work, and an upload is
      ...
      may touch. → T018
- [x] **T020** `[docs]` Run the full verification pass — `bin/cli api npx --no tsc --noEmit`,
