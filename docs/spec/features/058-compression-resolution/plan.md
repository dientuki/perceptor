...
cases; `web` build exits 0 and `check-messages` reports no drift; `worker` typecheck reports only
the 2 pre-existing `src/metadata/container-tags.spec.ts` errors (re-confirm at `HEAD` first) and
