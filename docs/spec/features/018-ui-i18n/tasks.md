...
- [x] **T001** `[api] [P]` Add `User.uiLocale`, `MediaSource.errorKey`/`errorParams` and
...
- [x] **T002** `[api] [P]` Write `src/i18n/locales.ts` (`SUPPORTED_LOCALES`, `DEFAULT_LOCALE`),
...
- [x] **T003** `[api]` Write `src/i18n/i18n-error.ts` — one factory per Nest exception type used in
      ...
      *Done when:* `i18nError.notFound(ERROR_KEYS.MOVIE_NOT_FOUND, { id: 1 })` returns a
      ...
      object.
- [x] **T004** `[api]` Write `src/i18n/graphql-error.formatter.ts`, wire it into the existing
...
- [x] **T005** `[api]` Add `uiLocale` to `User` entity and the `me` selection; add
...
- [x] **T006** `[api] [P]` Expose `errorKey`/`errorParams` on
...
- [x] **T007** `[api] [P]` Change `encodeFailed` to
...
- [x] **T008** `[api]` Key the five auth throw sites (`auth.resolver.ts`, `auth.service.ts`,
      ...
      *Done when:* every auth throw site emits `error.auth.unauthenticated` or
      ...
      them.
- [x] **T009** `[api] [P]` Key the throw sites in `users`, `movies`, `shows`, `seasons`, `episodes`
...
- [x] **T010** `[api] [P]` Key the throw sites in `media-roots`, `media-sources`, `process-jobs`,
...
- [x] **T011** `[api] [P]` Move the eight `class-validator` constraint messages to keys and extend
...
- [x] **T012** `[api]` Write `errorKey`/`errorParams` alongside `errorMessage` wherever `api`
...
- [x] **T013** `[api]` Give `UploadHttpError` the key/params pair, serialize `{ message, i18n }` in
      ...
      *Done when:* an expired ticket returns HTTP 401 with `i18n.key =
      ...
      in `src/uploads/`.
- [x] **T014** `[api] [P]` Rewrite `src/languages/language-names.ts` in English and drop the
...
- [x] **T015** `[api]` Update `src/media-roots/media-roots.service.spec.ts` and
...
- [x] **T016** `[web]` Install `next-intl`, `negotiator` and `@formatjs/intl-localematcher` via
...
- [x] **T017** `[web]` Write `src/i18n/locales.ts` and `src/i18n/negotiate.ts` — language-range
...
- [x] **T018** `[web]` Extract a non-redirecting, `cache()`-wrapped `getCurrentUserOrNull()` in
...
- [x] **T019** `[web]` Write `src/i18n/request.ts` implementing REQ-1's order: user row → header →
      `en`. → T017, T018
      *Done when:* the resolver returns the user's locale when set, the negotiated header locale
      ...
      redirecting**.
- [x] **T020** `[web]` Make `src/app/layout.tsx` async: resolve the locale, set `<html lang>`, wrap
...
- [x] **T021** `[web]` Write `src/lib/graphql-error.ts`: read `extensions.i18n.key`, `JSON.parse`
...
- [x] **T022** `[web]` Migrate `src/lib/auth-session.ts` to match `error.auth.unauthenticated` and
...
- [x] **T023** `[web]` Rewrite the twelve `src/actions/*.ts` to derive error strings through
...
- [x] **T024** `[web] [P]` Add `src/actions/locale.ts` with `setUiLocaleAction`, copying
...
- [x] **T025** `[web] [P]` Extract every literal in `src/app/**`, `src/layout/**` and
      ...
      *Done when:* no user-facing literal remains in those directories and both catalogs carry
      every new key.
- [x] **T026** `[web] [P]` Extract every literal in `src/components/**` (search, import, settings,
      ...
      *Done when:* a search for Spanish copy under `src/components` returns nothing outside the
      catalogs (AC-9).
- [x] **T027** `[web]` Render language names with `Intl.DisplayNames(activeLocale)`, sort with
...
- [x] **T028** `[web]` Read `i18n.key` off the REST error body from `/uploads` in
...
- [x] **T029** `[web]` Write `scripts/check-messages.mjs` — exit non-zero listing any key present in
...
- [x] **T030** `[worker] [P]` Write `src/i18n/error-keys.ts`, `src/i18n/messages.en.ts` and
...
- [x] **T031** `[worker] [P]` Convert the throw sites in `ffmpeg/params.ts`, `ffmpeg/metadata.ts`,
...
- [x] **T032** `[worker]` Change `src/api/graphql-client.ts:50` to propagate an incoming
      ...
      *Done when:* an `api` error surfaced through the worker carries its original key rather than a
      JSON blob.
- [x] **T033** `[worker]` Rewrite `src/jobs/encode.job.ts:139-144` for the four-argument
...
- [x] **T034** `[docs]` Add an `018-ui-i18n` section to `docs/spec/graphql-contract.md`: the schema
...
- [x] **T035** `[docs]` Update the root `CLAUDE.md` (Conventions — user-facing copy is no longer
      ...
      changes status.
- [x] **T036** `[docs]` Walk all twelve acceptance criteria in `spec.md` — including the AC-8
