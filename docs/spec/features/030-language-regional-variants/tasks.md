...
- [x] **T001** `[api]` In `services/api/prisma/schema.prisma`, add `tag String @unique` to
...
- [x] **T002** `[api]` Rewrite `services/api/prisma/seeds/languages.ts`: every existing row gains a
...
- [x] **T003** `[api]` Add `@Field() tag: string` to
...
- [x] **T004** `[api]` In `LanguagesService.findAll()`
...
- [x] **T005** `[api]` Point `validateAndResolveLanguageIds` at `tag` — the parameter, the
...
- [x] **T006** `[api]` Rename `@Args('iso2')` to `@Args('tags')` on `setMoviePreferredLanguages`
...
- [x] **T007** `[api] [P]` Update `src/settings/settings.service.spec.ts` to exercise the
...
- [x] **T008** `[api] [P]` Replace `ProcessJobsService.resolveIso3(iso2)` with
...
- [x] **T009** `[api]` Add `@Field(() => [String]) allowedLanguageTags: string[]` to
...
- [x] **T010** `[web]` Add `tag: string` to `src/types/languages.ts`, then add `tag` to every
...
- [x] **T011** `[web] [P]` In `src/actions/languages.ts`, rename `$iso2` → `$tags` in **both**
...
- [x] **T012** `[web] [P]` Rewrite `src/components/media/LanguagePicker.tsx` as the shared dual-pane
...
- [x] **T013** `[web] [P]` In `src/components/settings/DownloadPanel.tsx`, replace `MultiSelect`
...
- [x] **T014** `[web] [P]` Pass `name="tags"` to the picker from
...
- [x] **T015** `[web] [P]` Add `errors.language.unavailable` and `errors.language.duplicate` to
...
- [x] **T016** `[docs]` Update `docs/spec/graphql-contract.md` § "Language preferences drive the
...
- [x] **T017** `[docs]` Update `services/api/CLAUDE.md` (the `languages/` and `process-jobs/`
