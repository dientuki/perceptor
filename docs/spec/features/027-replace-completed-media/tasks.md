...
- [ ] **T001** `[api]` Add `MOVIE_ALREADY_COMPLETED`, `EPISODE_ALREADY_COMPLETED` and
      ...
      *Done when:* `bin/cli api npx --no tsc --noEmit` reports 0 errors and
      ...
      file.
- [ ] **T002** `[api] [P]` In `MoviesService.attachTorrentSource` (`src/movies/movies.service.ts`),
...
- [ ] **T003** `[api] [P]` The same branch in `EpisodesService.attachTorrentSource`
...
- [ ] **T004** `[api] [P]` The same branch in `SeasonsService.attachTorrentSource`
      ...
      *Done when:* `addMagnetToSeason` against a season with at least one `COMPLETED` episode answers
      ...
      (**AC-10**).
- [ ] **T005** `[api]` Write `src/movies/movies.service.spec.ts` — **defends against the wrong
...
- [ ] **T006** `[api]` In `src/uploads/upload-tickets.service.ts`, add `force?: boolean` to
...
- [ ] **T007** `[api]` In the same file, add the replace marker: a `UPLOAD_REPLACE_KEY_PREFIX`
...
- [ ] **T008** `[api]` In `src/uploads/uploads.resolver.ts`, add
...
- [ ] **T009** `[api]` In `UploadsService.onUploadCreate` (`src/uploads/uploads.service.ts`),
...
- [ ] **T010** `[api]` In `UploadsService.handleUploadFinish`, in **both** the episode and the film
...
- [ ] **T011** `[api]` Extend `src/uploads/upload-tickets.service.spec.ts` — **defends against a
...
- [ ] **T012** `[api]` Boot the service and confirm the regenerated `src/schema.gql`. → T008
...
- [ ] **T013** `[web] [P]` Add `toActionError(error)` to `src/lib/graphql-error.ts`, returning
...
- [ ] **T014** `[web] [P]` Add `AcquisitionResult` to `src/types/media.ts`:
...
- [ ] **T015** `[web] [P]` **Add** to both `messages/en.json` and `messages/es.json`: the six
...
- [ ] **T016** `[web]` Change `importMagnetAction` (`src/actions/imports.ts`),
...
- [ ] **T017** `[web]` Add `$force: Boolean` to `CREATE_UPLOAD_TICKET_MUTATION` and a `force = false`
...
- [ ] **T018** `[web]` Create `src/components/import/ReplaceWarning.tsx` — a presentational client
...
- [ ] **T019** `[web]` Rework `src/components/import/importMagnetModal.tsx`: derive `isCompleted`
...
- [ ] **T020** `[web]` Rework `src/components/import/importFileModal.tsx`: when `isCompleted` and the
...
- [ ] **T021** `[web]` Rework `src/components/search/SearchTorrent.tsx`: render `<ReplaceWarning />`
...
- [ ] **T022** `[web]` Delete `search.torrent.conflictMarker` and `import.magnet.conflictMarker` from
...
- [ ] **T023** `[docs]` Update `services/api/CLAUDE.md` (the uploads section: `createUploadTicket`
...
- [ ] **T024** `[docs]` Walk the acceptance criteria in `spec.md` against the running stack, using
