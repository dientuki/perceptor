...
- [ ] **T001** `[infra]` Add `INDEXER_API_KEY=` (empty) to `.env.example`, immediately after
...
- [ ] **T002** `[infra]` In `bin/install`, add the adopt-or-generate block after the `JWT_SECRET`
...
- [ ] **T003** `[infra]` Create `services/indexer/custom-cont-init.d/10-prowlarr-apikey` and add
...
- [ ] **T004** `[infra]` In `docker-compose.yaml`: add the `flaresolverr` service
...
- [ ] **T005** `[infra]` Add `flaresolverr` to the `SERVICES` string in `bin/dev` and `bin/prod`. → T004
      *Done when:* `bin/dev` brings up nine services and `docker compose ps` lists all nine (AC-1).

- [ ] **T006** `[infra]` Create `services/indexer/custom-services.d/20-prowlarr-flaresolverr`:
...
- [ ] **T007** `[api] [P]` In `services/api/prisma/seeds/settings.ts`, seed `tracker_api_key` from
...
- [ ] **T008** `[api] [P]` In `services/api/src/clients/indexer/client.ts`, make `getData` check the
...
- [ ] **T009** `[api]` Create `services/api/src/clients/indexer/client.spec.ts`, opening with a
...
- [ ] **T010** `[web]` In `services/web/src/components/search/SearchTorrent.tsx`, stop swallowing the
...
- [ ] **T011** `[docs]` Update the root `CLAUDE.md`: the **Find release (indexer)** pipeline row (no
...
- [ ] **T012** `[docs]` Walk every acceptance criterion in `spec.md` and tick it. AC-5 and AC-7 are
