...
- [ ] **T001** `[verify]` Dump the argument array `buildFfmpegCommand` produces for all **eight**
...
- [ ] **T002** `[ffmpeg]` `src/ffmpeg/params.ts`: delete `SOFTWARE_TONEMAP_VF`, `GPU_TONEMAP_VF` and
...
- [ ] **T003** `[ffmpeg]` `src/ffmpeg/buildCommand.ts`: remove the `vulkanAvailable` parameter and
...
- [ ] **T004** `[ffmpeg] [P]` Delete `src/ffmpeg/vulkan.ts` and `src/ffmpeg/vulkan.spec.ts`.
      *Done when:* neither file exists. → T001
- [ ] **T005** `[ffmpeg]` `src/ffmpeg/cases.spec.ts` **and all eight corpus JSONs in one change**:
...
- [ ] **T006** `[ffmpeg]` Correct the stale `ffmpeg` expectation in `1.json`, `3.json` and `4.json` —
...
- [ ] **T007** `[ffmpeg] [P]` Delete the `getVideoParams — 4K HDR tonemap branches` describe block in
...
- [ ] **T008** `[worker]` Remove the two call sites: the `isVulkanAvailable` import, the memo comment
...
- [ ] **T009** `[infra] [P]` Delete `docker-compose.gpu.yaml` and remove the
...
- [ ] **T010** `[infra] [P]` Remove `- USE_GPU=${USE_GPU}` from the `worker` service in
      ...
      *Done when:* `grep -rn USE_GPU docker-compose.yaml docker-compose.dev.yaml .env.example`
      returns nothing.
- [ ] **T011** `[infra] [P]` Drop `vulkan-loader`, `mesa-vulkan-intel` and `mesa-vulkan-ati` from the
...
- [ ] **T013** `[verify]` Re-dump all eight argument arrays with the new four-argument signature and
      `diff` against T001.
...
- [ ] **T014** `[ffmpeg]` `git rm services/worker/ffmpeg/{1,2,3,4,5,6,7}.json` and
...
- [ ] **T015** `[verify]` `bin/build worker && bin/dev`, then read the logs and the container. The
...
- [ ] **T016** `[verify]` Add `USE_GPU=false` back into `.env` by hand and confirm `bin/dev`'s
...
- [ ] **T017** `[docs]` `.claude/agents/ffmpeg.md` — **the highest-value task in this feature.**
      ...
      *Done when:* the file cites no case file that does not exist, and V3 describes HDR preserved.
      → T014
- [ ] **T018** `[docs] [P]` `services/worker/CLAUDE.md` — the last *Known debt* bullet (Vulkan
...
- [ ] **T019** `[docs] [P]` Root `CLAUDE.md` — the transcode row of the pipeline table (1080p with
...
- [ ] **T020** `[docs] [P]` `README.md` — the transcode feature bullet (`:48`), the GPU-acceleration
      ...
      *Done when:* the GPU-acceleration bullet is gone and the transcode bullet says HDR is
      preserved. → T014
- [ ] **T021** `[verify]` Run the AC-3 sweep, last, because it covers the prose the four tasks above
      just corrected:
...
- [ ] **T022** `[docs]` Walk the acceptance criteria in `spec.md`, tick each box against the evidence
