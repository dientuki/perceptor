...
- [ ] **REQ-1 (No Vulkan probe)**: The worker must not probe for, detect, memoize or log the
...
- [ ] **REQ-2 (No GPU flag in the encode path)**: No function that builds FFmpeg arguments may
...
- [ ] **REQ-3 (No tonemap chains)**: The `libplacebo` filter chain and the `zscale`/`tonemap`
      software chain must not appear anywhere in `services/worker/src/`. The worker emits no
      tonemapping filter for any source.
- [ ] **REQ-4 (The image carries no Vulkan stack)**: The `worker` image must not install a Vulkan
      loader or any Mesa Vulkan driver. Nothing in the image exists to serve a filter the worker
      never emits.
- [ ] **REQ-5 (No device mapping, no branch)**: The GPU compose overlay must be gone, and
      ...
      any container.
- [ ] **REQ-6 (`USE_GPU` retired)**: `USE_GPU` must be removed from `.env.example` and read by no
...
- [ ] **REQ-7 (The suite is green and honest)**: `bin/npm worker test` must pass with no failures.
...
- [ ] **REQ-8 (The corpus is reset to one current case)**: The seven versioned cases
...
- [ ] **REQ-9 (Documentation stops promising SDR)**: Every live documentation surface that today
...
- [ ] **REQ-10 (`017` is superseded, not erased)**: `docs/spec/features/017-worker-gpu-strategy/`
...
- [ ] **NFR-1 (Not one encode changes)**: This is removal only. For every source the corpus covers,
      ...
      Article IX).
- [ ] **NFR-2 (Starts everywhere, unconditionally)**: The `worker` container must start and complete
...
- [ ] **NFR-3 (Nothing kept for later)**: No replacement flag, no `HW_ACCEL` variable, no commented
...
- [ ] **NFR-4 (The diff removes)**: The net change must delete substantially more than it adds. An
...
- [ ] **AC-1**: `bin/npm worker test` reports **0 failed**. Today it reports `Test Files 3 failed |
      10 passed (13)` and `Tests 11 failed | 106 passed (117)`; that exact output is the before-state
      this criterion closes.
- [ ] **AC-2**: `bin/cli worker npx --no tsc --noEmit` reports 0 errors.
- [ ] **AC-3**: `grep -rniE 'vulkan|libplacebo|tonemap|USE_GPU|dev/dri' services/worker bin
...
- [ ] **AC-4 (regression guard, NFR-1)**: For all **eight** probes present in
...
- [ ] **AC-5 (corpus reset)**: `services/worker/ffmpeg/` contains exactly one file, `8.json`, and it
...
- [ ] **AC-6 (failure path)**: On a host with **no** `/dev/dri`, `bin/dev` brings the `worker` to a
...
- [ ] **AC-7 (failure path)**: With a stale `USE_GPU=false` still present in `.env`, `bin/dev`
...
- [ ] **AC-8**: `bin/install` run against a fresh checkout asks nothing about a GPU and produces an
      `.env` in which `grep -c USE_GPU .env` returns `0`.
- [ ] **AC-9**: `docs/spec/features/017-worker-gpu-strategy/spec.md` reads `status: Superseded` and
