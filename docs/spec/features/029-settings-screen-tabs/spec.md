...
- [x] **REQ-1 (Tabs)**: `/settings` must present its content as six tabs — General, Media Manager,
      ...
      selected on load.
- [x] **REQ-2 (Tab contents)**: The controls must be distributed as follows, and no control may
      appear in two tabs:
...
- [x] **REQ-3 (One Save)**: The screen must have exactly one Save control. Activating it must submit
...
- [x] **REQ-4 (Shared input components)**: Every checkbox on the screen must be
...
- [x] **REQ-5 (Default UI language is a setting)**: The installation's default UI language must be
...
- [x] **REQ-6 (Locale resolution order)**: `web` must resolve the UI locale as
...
- [x] **REQ-7 (Default download languages is a setting)**: The default download languages must be an
...
- [x] **REQ-8 (Encode merge)**: The set of languages an encode may keep must be the title's original
...
- [x] **REQ-9 (Compresión is markup)**: The Compresión tab must render its toggle and radios and must
...
- [x] **REQ-10 (No orphan save path)**: The separate download-languages card and its own save action
...
- [x] **REQ-11 (Settings are administrator-only)**: Reading and writing installation-wide settings
...
- [x] **NFR-1 (No backfill)**: Dropping the per-user global download-language level discards what
      ...
      before they go.
- [x] **NFR-2 (All-or-nothing save)**: A rejected value anywhere on the screen must leave the stored
...
- [x] **NFR-3 (No new anonymous surface beyond the locale)**: The installation default UI language
...
- [x] **NFR-4 (Error copy is catalog-driven)**: Every new error crosses the boundary as English text
...
- [x] **AC-1**: `/settings` renders six underlined tabs with icons; clicking **Media Server** shows
      the client/host/port/API-key fields and hides the folder pickers, and General is the panel
      shown on first load.
- [x] **AC-2**: With General set to a new language, Media Manager's movies folder changed, and
...
- [x] **AC-3**: `bin/mysql -e "select \`key\`, value from settings where \`key\` in ('ui_locale','default_languages')"`
      returns both rows after a save, with `default_languages` holding the codes chosen in Descarga
      in the order they were chosen.
- [x] **AC-4 (failure path)**: With Media Manager's movies folder set to `../etc` **and** Descarga
...
- [x] **AC-5 (failure path)**: A `updateSettings` call carrying `{ key: "ui_locale", value: "de" }`
      ...
      clears the list.
- [x] **AC-6**: With `ui_locale` set to `es` and the browser sending `Accept-Language: en-US`, a
...
- [x] **AC-7**: After the migration, `bin/mysql -e 'show tables like "user_languages"'` returns
      nothing and `default_languages` holds the empty string until an administrator saves the
      Descarga tab.
- [x] **AC-8**: A film whose owner has a per-title preference of `fr` is queued for encode while
...
- [x] **AC-9 (failure path)**: Signed in as a non-administrator, `/settings` is absent from the
...
- [x] **AC-10**: `bin/npm web run build` exits 0 and `bin/npm api run test` is green.
