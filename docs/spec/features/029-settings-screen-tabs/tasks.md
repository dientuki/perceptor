...
- [ ] **T001** `[api]` Relax `SettingInput.value` in
...
- [ ] **T002** `[api] [P]` Add `'languages'` to `SettingKind` and the two entries to
...
- [ ] **T003** `[api] [P]` Promote `LanguagesService.resolveLanguageIds` in
...
- [ ] **T004** `[api]` Import `LanguagesModule` in `services/api/src/settings/settings.module.ts`
      ...
      *Done when:* `bin/cli api npx --no tsc --noEmit` is clean and the api container boots without
      ...
      initialized`).
- [ ] **T005** `[api]` In `SettingsService.updateMany`
...
- [ ] **T006** `[api]` Write `services/api/src/settings/settings.service.spec.ts`, opening with the
...
- [ ] **T007** `[api]` In `services/api/src/settings/settings.resolver.ts`, apply
...
- [ ] **T008** `[api]` Write `services/api/src/settings/settings.resolver.spec.ts` with the
      ...
      *Done when:* `bin/npm api run test` is green, and the suite fails if `AdminGuard` is moved to
      the class.
- [ ] **T009** `[api] [P]` Append `{ key: 'ui_locale', value: '' }` and
...
- [ ] **T010** `[api]` In `services/api/src/process-jobs/process-jobs.service.ts`, make
      ...
      *Done when:* `bin/cli api npx --no tsc --noEmit` is clean and no `user.languages` select
      remains in the file.
- [ ] **T011** `[api]` Add the merge case to
      ...
      *Done when:* `bin/npm api run test` is green and the case fails if the `default_languages`
      read is removed.
- [ ] **T012** `[api]` Remove `setPreferredLanguagesFor` and `findPreferredLanguagesFor` from
...
- [ ] **T013** `[api]` Remove the `preferredLanguages` `@ResolveField` from
...
- [ ] **T014** `[api]` Remove the `UserLanguage` model and the `User.languages` relation from
...
- [ ] **T015** `[web] [P]` Create `services/web/src/components/ui/tabs/TabNav.tsx`: a presentational
...
- [ ] **T016** `[web]` In `services/web/src/actions/settings.ts`: add `getDefaultUiLocale()`
...
- [ ] **T017** `[web]` In `services/web/src/i18n/request.ts`, insert the installation default
...
- [ ] **T018** `[web]` Restructure `services/web/src/components/settings/SettingsForm.tsx` into one
...
- [ ] **T019** `[web]` Add `GeneralPanel.tsx` (the app-language select, `name="ui_locale"`) and
...
- [ ] **T020** `[web]` Add `CompressionPanel.tsx`: one `components/form/input/Checkbox.tsx` and a
...
- [ ] **T021** `[web]` In `services/web/src/app/(dashboard)/settings/page.tsx`, check
...
- [ ] **T022** `[web]` Move the `/settings` entry into the `isAdmin`-only branch in
...
- [ ] **T023** `[web]` Delete `setPreferredLanguagesAction` from
...
- [ ] **T024** `[web]` Add the new copy to `services/web/messages/en.json` and `messages/es.json` —
...
- [ ] **T025** `[api]` Run the api verification pass and fix anything it surfaces within `api`.
      → T014
...
- [ ] **T026** `[web]` Run the web verification pass and fix anything it surfaces within `web`.
      → T024
...
- [ ] **T027** `[docs]` Update `docs/spec/graphql-contract.md`: record the removal of
...
- [ ] **T028** `[docs]` Update `services/api/CLAUDE.md` (the `settings/` and `languages/` module-map
...
- [ ] **T029** `[docs]` Walk the ten acceptance criteria in `spec.md`, including the manual pass in
