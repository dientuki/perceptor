...
- [x] **T001** `[api]` Move `prisma` from `devDependencies` to `dependencies` in
...
- [x] **T002** `[api]` Add `url = env("DATABASE_URL")` to the `datasource db` block in
      ...
      *Done when:* `bin/cli api npx --no prisma migrate status` reports the schema up to date, the
      ...
      directory.
- [x] **T003** `[api]` Create `services/api/src/database/seed/production-seed.ts` calling
...
- [x] **T004** `[api]` Backfill `movie_db_api_key` from `TMDB_API_KEY` in
...
- [x] **T005** `[api]` Create `services/api/src/bootstrap/run-migrations.ts`: spawn the Prisma CLI
...
- [x] **T006** `[api]` Wire T005 and the production seed into `services/api/src/main.ts`, after
...
- [x] **T007** `[api] [P]` Write `services/api/src/database/seed/production-seed.spec.ts` covering
      ...
      *Done when:* `bin/npm api test` passes and the suite fails if the loop is changed to a bare
      `upsert`.
- [x] **T008** `[api] [P]` Write `services/api/src/bootstrap/run-migrations.spec.ts` driving the
...
- [x] **T009** `[infra]` Split the compose files. Create `docker-compose.build.yaml` holding the
...
- [x] **T010** `[infra]` Add `restart: unless-stopped` to `web`, `api` and `worker`, and put
      ...
      *Done when:* `docker inspect` reports the restart policy on all three; `docker compose up -d`
      ...
      still starts it.
- [x] **T011** `[infra]` Ship the AutoRun hook inside the image: add `COPY --chmod=755 commands/
...
- [x] **T012** `[infra]` Add the one-shot `backup` service (`mariadb:12.3.2`, `restart: "no"`,
...
- [x] **T013** `[infra]` Pass `TMDB_API_KEY` and `PERCEPTOR_AUTO_MIGRATE` through to `api` in
      `docker-compose.yaml`. → T009
      *Done when:* `docker compose exec api printenv TMDB_API_KEY PERCEPTOR_AUTO_MIGRATE` prints
      both values.
- [x] **T014** `[infra]` Write `.github/workflows/release.yml`: on a `v*` tag, build the five
...
- [x] **T015** `[infra]` Write `install.sh` at the repository root — fetched by `curl` into an empty
...
- [x] **T016** `[infra]` Update `.env.example` with `PERCEPTOR_TAG`, `TMDB_API_KEY`,
...
- [x] **T017** `[docs]` Update the root `CLAUDE.md` — the `bin/` wrapper table and § Layout for
...
- [x] **T018** `[docs]` Walk AC-1 through AC-13 in `spec.md`, tick each box, and set
