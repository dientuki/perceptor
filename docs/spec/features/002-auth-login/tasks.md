...
- [x] **T001** `[api]` Create `src/auth/auth.constants.ts` with `AUTH_COOKIE_NAME = 'auth_token'`,
...
- [x] **T002** `[api]` Create `src/auth/auth.types.ts`: the `AuthPrincipal` union
...
- [x] **T003** `[api]` Create `src/auth/session.service.ts` over the existing `RedisService`:
...
- [x] **T004** `[api]` Rewrite `src/auth/strategies/jwt.strategy.ts`: `jwtFromRequest:
...
- [x] **T005** `[api]` Create `src/auth/decorators/public.decorator.ts` (`IS_PUBLIC_KEY`,
...
- [x] **T006** `[api]` Apply the two decorators. `@Public()` on the `login` mutation in
...
- [x] **T007** `[api]` The contract changes. `src/auth/dto/login.input.ts` gains
...
- [x] **T008** `[api]` Upload tickets. `src/uploads/entities/upload-ticket.entity.ts`
...
- [x] **T009** `[api]` Create `services/api/scripts/mint-service-token.ts` — reads `JWT_SECRET`
...
- [x] **T010** `[orch]` Add `JWT_SECRET` and `SERVICE_TOKEN` to `.env.example` with placeholder
...
- [x] **T011** `[worker] [P]` In `src/api/graphql-client.ts`: read `process.env.SERVICE_TOKEN`
...
- [x] **T012** `[orch] [P]` Rewrite the request in
...
- [x] **T013** `[web] [P]` Rename `CONFIG.authTtoken` → `CONFIG.authCookie` in
...
- [x] **T014** `[web]` In `src/lib/graphql-client.ts`: read `cookies()` inside `fetchGraphQL` and
...
- [x] **T015** `[web]` Call `redirectIfUnauthenticated(errors)` immediately after each of the 11
...
- [x] **T016** `[web]` Wire "keep me logged in" end to end: add an optional `name?: string` prop to
...
- [x] **T017** `[web]` Add `logoutAction` to `src/actions/auth.ts`: call the `logout` mutation
...
- [x] **T018** `[web]` Consume `me`. Add `getCurrentUser()` to `src/actions/auth.ts` wrapping the
...
- [x] **T019** `[web]` Create `src/actions/uploads.ts` with `createUploadTicketAction(movieId)`.
...
- [x] **T020** `[api]` Register `{ provide: APP_GUARD, useClass: JwtAuthGuard }` in
...
- [x] **T021** `[docs]` Update the documentation the feature invalidates: the root `CLAUDE.md`
...
- [x] **T022** `[docs]` Walk all thirteen acceptance criteria in `spec.md`, ticking each box only
