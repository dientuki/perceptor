...
| :-- | :-- | :-- | :-- |
| AC-9 (partial) | web | `services/web/src/lib/graphql-error.ts`'s `translateGraphQLError` does `JSON.parse(rawParams)`, but `api`'s `i18nError.*` factories (`services/api/src/i18n/i18n-error.ts`) attach `extensions.i18n.params` as a raw JS object, never `JSON.stringify`'d. `JSON.parse` on a non-string throws, so **every** keyed error that carries params (`error.season.not_found` `{id}`, `error.magnet.already_attached` `{title}`) silently falls back to the English `message` regardless of UI locale — confirmed live on both the season path (this feature) and the pre-existing movie path (`addMagnetToMovie`), so it predates `059` and is not a regression it introduced. A param-less key (`error.magnet.not_a_magnet`, tested live in `es`) translates correctly. | Fix belongs in `018-ui-i18n`'s territory (`graphql-error.ts` and/or `i18n-error.ts`'s wire shape), not in any `059` file — neither `api/plan.md` nor `web/plan.md` touch either file, so no `059` task owns it. Needs its own fix, either stringifying `params` on the `api` side or dropping the `JSON.parse` on the `web` side. |
