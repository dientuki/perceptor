...
- [ ] **T001** `[api]` Add `enum ContentKind { LIVE_ACTION ANIME CGI }` to `prisma/schema.prisma`,
...
- [ ] **T002** `[api] [P]` Create `src/media/entities/content-kind.enum.ts` — the TS enum plus
...
- [ ] **T003** `[api] [P]` Create `src/media/content-kind.ts` — `classifyContentKind({ genreIds,
...
- [ ] **T004** `[api]` Teach `TmdbClient` genres and keywords: map `genres` → `genreIds` on both
...
- [ ] **T005** `[api]` `MoviesService.register()`: resolve the catalog entry, top it up with **one**
...
- [ ] **T006** `[api]` `ShowsService.register()`: the same derivation on the series path (no runtime),
...
- [ ] **T007** `[api]` Add `MoviesService.setContentKind` / `ShowsService.setContentKind` and expose
...
- [ ] **T008** `[api]` Replace `isLiveAction` with `contentKind` on
...
- [ ] **T009** `[api]` Finish the rename across `src/movies/entities/movies.entity.ts`,
...
- [ ] **T010** `[web] [P]` Rename the selected field in `src/actions/movies.ts` and
...
- [ ] **T011** `[web]` Add the `contentKind` namespace (three labels) plus `contentKindLabel` under
...
- [ ] **T012** `[web]` Create `src/components/media/ContentKindSelect.tsx` (one renderable export, on
...
- [ ] **T013** `[worker] [P]` Create `src/encode/content-kind.ts` — the worker-local
...
- [ ] **T014** `[worker]` Change `EncodeInput.isLiveAction` to `contentKind: ContentKind` (required)
      ...
      *Done when:* `bin/cli worker npx --no tsc --noEmit` reports only the 2 known pre-existing
      ...
      kind.
- [ ] **T015** `[worker]` **Dispatch to the `ffmpeg` agent.** In `src/ffmpeg/params.ts`, take the
      ...
      *Done when:* `bin/npm worker test -- params` and `-- buildCommand` are green, with one case per
      ...
      of `enable-qm`.
- [ ] **T016** `[worker]` **Dispatch to the `ffmpeg` agent.** Make `src/ffmpeg/cases.spec.ts`
...
- [ ] **T017** `[docs]` Record the delta in `docs/spec/graphql-contract.md`: the `ContentKind` enum,
...
- [ ] **T018** `[docs]` Update the root `CLAUDE.md` pipeline table (the **Register title in DB** and
...
- [ ] **T019** `[docs]` Run the cross-service verification sweep in `plan.md` § Verification — every
...
- [ ] **T020** `[docs]` Tick each acceptance criterion in `spec.md`, tick the requirement boxes, and
